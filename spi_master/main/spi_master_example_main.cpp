...
void get_picture_Lines(uint16_t *dest, int line, int frame, int linect);
...
static void display_pretty_colors(spi_device_handle_t spi)
{
    ...
    while(1) {
        ...
        for (int y=0; y<240; y+=PARALLEL_LINES) {
            ...
            get_picture_Lines(lines[calc_line], y, frame, PARALLEL_LINES);
            //memset(lines[calc_line],frame,320*PARALLEL_LINES*sizeof(uint16_t));
            //memset(lines[1],frame,320*PARALLEL_LINES*sizeof(uint16_t));
...
bool display_Init()
{
    ...
    Image_buf = (uint16_t *)malloc(320*240*sizeof(uint16_t));
...
//impressive. Because the SPI driver handles transactions in the background, we can calculate the next line
//while the previous one is being sent.
static void display_picture(spi_device_handle_t spi)
{
    uint16_t *lines[2];
    //Allocate memory for the pixel buffers
    for (int i=0; i<2; i++) {
    ...
    while(1) {
        ...
        for (int y=0; y<240; y+=PARALLEL_LINES) {
            //Calculate a line.
            //pretty_effect_calc_lines(lines[calc_line], y, frame, PARALLEL_LINES);
            memset((uint8_t *)lines[0],0,(int )(320*PARALLEL_LINES*sizeof(uint16_t)));
            memset((uint8_t *)lines[1],0x28,(int )(320*PARALLEL_LINES*sizeof(uint16_t)));
            //Finish up the sending process of the previous line, if any
            if (sending_line!=-1) send_line_finish(spi);
            //Swap sending_line and calc_line
            sending_line=calc_line;
...
void Bilinear(uint16_t* inpBmpBuf, int src_x,int src_y,int dst_x,int dst_y,int dstWidth,int dstHeight)
{
            ...
            for (int j = 0; j < dstWidth; ++j){
                ...
                if (Fp >= 0 && Fp <= 65535){
                    ...
                    if(round(Fp)>34000)
                    {
                        ...
                        set_Image_pixel( dst_x + i, dst_y + j,0xFFFF);
                    }
                    else
                    {
                        set_Image_pixel( dst_x + i, dst_y + j,0x0);
                    }
...
static void display_QRCODE(spi_device_handle_t spi)
{
...
void SPI_LCD_init()
{
    ...
    spi_bus_config_t buscfg={0};
        ...
        buscfg.max_transfer_sz=(int)(PARALLEL_LINES*320*2+8);
...
#endif
        devcfg.mode=(uint8_t)0;                                //SPI mode 0
